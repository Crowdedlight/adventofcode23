...
impl Hand {
    pub fn new(cards: Vec<char>, bid: u64) -> Self {
        ...
        let mut hand_type = get_type(&cards);

        // optimize hand_type if hand have jacks
        if cards.contains(&'J') && hand_type != HandType::FiveOfKind {
            // try each replacement card option
            // TODO learned after the fact that I only need to check for upgrades based on other chars already existing in the array, as it will only be those that will upgrade...
            let options = vec!['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2'];
            for card in options {
                // replace Jokers with new card
                let new_hand: Vec<char> = cards.iter().map(|&x| if x == 'J' {card} else {x}).collect();
                let new_hand_type = get_type(&new_hand);
